...
/*根据现在运行的情况来看，既然能调控CPU转速的话，那么上面部分参数应该是没有问题的*/
/* EC registers can be read by EC_SC_READ_CMD or /sys/kernel/debug/ec/ec0/io:
 *
 * 1. modprobe ec_sys
 * 2. od -Ax -t x1 /sys/kernel/debug/ec/ec0/io
 */
/*对应参数模块，实现不同机器的控制应该修改此参数*/
#define EC_REG_SIZE 0x100//指定该分区的大小并判断是否符合条件
#define EC_REG_CPU_TEMP 0x07//对应读取CPU的温度的地址
#define EC_REG_GPU_TEMP 0xCD//对应读取GPU温度的地址，这台电脑是0xF9
#define EC_REG_FAN_DUTY 0xCE//对应CPU风扇的地址，和现在的情况相符
#define EC_REG_FAN_RPMS_HI 0xD0//猜测为CPU转速的最小值//显卡的是0xD2
#define EC_REG_FAN_RPMS_LO 0xD1//猜测为CPU转速的最大值//显卡的是0xD3
